...
const skillInstructions: Record<string, string> = {
  ...
  'chrome-browser-control': [
    ...
    'Use browser_snapshot before each decision you make directly and use only fresh accessibility refs from that snapshot. After an action, inspect the returned fresh snapshot before continuing.',
    'Chrome delivers no clicks or keys to a tab it is not showing. browser_open therefore puts a tab in front; if a tool reports that a tab is hidden, call browser_show once and continue — after being shown a tab stays drivable even while another tab is selected.',
    'When browser_fast is one of this session’s tools, prefer it for a narrow, routine browser subgoal — navigation, searching, opening a menu, choosing an obvious result, or a predictable UI sequence — and give it a semantic goal plus the exact values to type through input, never a list of clicks. browser_fast keeps the same fresh-snapshot-before-action rule internally and returns control as soon as the subgoal is complete or the next step stops being obvious.',
    'State one outcome per browser_fast call, not a rule system. Instructions with several conditional branches slow every decision and make them less accurate; when a page needs that much logic, prefer a control that encapsulates it, or split the work into several short subgoals.',
    'For a sustained interaction — a game, a canvas, a terminal, a paged list, a form with many identical steps — pass control: true and declare exactly the keys the interface needs through keys. Otherwise the fast loop stops as soon as the page stops changing, which a canvas never does, and it cannot press a key it was not offered.',
    'Independent subgoals can be delegated together through goals, one Browser Use session each, so the loops overlap instead of queueing; a single tab can only be driven once at a time. A repetitive run — paging, scrolling, stepping, holding a direction — is one decision that covers several actions through max_repeat.',
    'The fast decision model cannot see pixels: a screenshot attached to its state is ignored. For a canvas, a map, a chart, or any custom-rendered component, read the screenshot yourself with browser_snapshot and describe the state that matters through input.',
    'Do not repeat browser_snapshot and browser_act step by step for obvious multi-step navigation that browser_fast can run safely. When browser_fast returns completed, continue from its final snapshot. When it returns escalate, inspect the returned snapshot, reason about the obstacle yourself, take the necessary normal Browser Use action, and delegate again once the interaction is routine.',
    'browser_fast is an optimization only. If it is not among this session’s tools, or it escalates, the normal Browser Use tools are authoritative. Never call a browser tool that is not registered in this session.',
    ...
    'One click per turn through the main model is the slowest possible way to do routine work: a subgoal that takes several obvious steps — opening a menu, choosing an obvious result, filling a short form, moving through a wizard — belongs to browser_fast, which decides inside one call from observations that cost no image and hands back as soon as the next step stops being obvious.',
    'A file input is set, never clicked: a page cannot open a file picker by itself, so browser_act action=upload is what performs an upload, and a click on a control that uploads is refused and names that action. The ref may be the visible control — a styled button, a label, a drop area — because the input it uploads through is resolved underneath it. Use only local paths explicitly supplied by the user or created for the requested task. Use browser_download for a fresh link ref; after a page-triggered download, use browser_download_wait before reading or reporting the resulting file.',
    ...
    'A run that ends suspends its browser sessions instead of releasing them: the debugger stays on the tab between two steps of the same work, because detaching and re-attaching is what makes Chrome’s “is debugging this browser” bar appear and disappear under the person’s hands, and that bar changes the page’s height. A task that is genuinely over releases its tabs for real — when the person moves to another task, or when the task stops having runs — which takes the bar off and hands the tab back to them. Call browser_release yourself when the work is done and the tab is no longer needed, and set close_tab only when the person asked for it.',
    'Chrome shows an "…is debugging this browser" infobar for as long as the debugger is attached, and that bar takes height from the page, so the same page measures differently depending on whether it is showing. Attach and detach are what make it appear and disappear: a suspend between two steps of the same work keeps the debugger attached and a release is what removes the bar, so release when the work is done rather than between steps. Read geometry from the snapshot, never from pixels: a coordinate or height taken from a screenshot describes the layout that was on screen at that moment, which is a different page when that bar came or went in between.',
    'An editor that renders its own document — a rich-text or canvas editor, an online document, a presentation tool — usually exposes one editing surface instead of focusable fields: its cells and paragraphs answer "Element is not focusable", and clicking is what places the caret. Place the caret by clicking the innermost visible element of the target first (a table cell’s paragraph, not the cell), then send the text with clear: false. The default clear: true selects everything in that surface before typing and will delete the document’s content.',
  ...
  'computer-use-control': [
    ...
    'When desktop_elements is available, prefer it for a named control or a form: it returns each control with a ref, its role, title, value, and whether it can be pressed, needs no image, and reads text exactly. Act on a control with desktop_act action=press or set_value and that ref. A ref describes the control as it was when read — after the interface changes, read the window again, because desktop_act refuses a ref that no longer matches. A screenshot remains the way to handle what has no element tree (a canvas, a game, media, a custom-drawn surface) and to verify how something looks.',
    ...
    'A native dialog, another application, or a system permission sheet is this capability’s work, not a scripting job: use desktop_windows, desktop_elements and desktop_act. Do not hand-roll osascript or System Events to click and type for you — it needs coordinates guessed from pixels, it cannot be verified, and it is the slowest possible route to a step. If the desktop capability is not enabled for this task, say so and stop instead of improvising a replacement.',
