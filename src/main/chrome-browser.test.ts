...
test('persisted Browser Use attachments are released because model runs do not survive an app restart', async () => {
  ...
  try {
    ...
    assert.deepEqual(await service.listAll(), [])
    ...
    const released = new Promise<number>(resolve => {
      ...
      client.on('message', raw => {
        ...
        if (request.method !== 'tab.release') return
        client!.send(JSON.stringify({ id: request.id, result: true }))
        resolve(request.params.tabId)
      })
    })
    await once(client!, 'open')
    client!.send(JSON.stringify({ type: 'hello', version: '1.0.0' }))
    assert.equal(await released, 42)
  } finally {
    client?.close()
    await service?.stop()
...
test('Chrome bridge owns claimed tabs per task, persists latest evidence, and releases without closing by default', async () => {
  ...
  try {
    ...
    assert.equal((await service.list('task-a')).some(item => item.id === runSession.id), false)
