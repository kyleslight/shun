...
export function capabilityPrompt(activeTools: string[], context: { workspaceSelected?: boolean } = {}) {
  ...
  if (activeTools.includes('browser_debug')) {
    ...
    lines.push('Opening a page for the user — including an external sign-in or authorization page in Browser Preview — is allowed and expected: the user completes credentials and consent themselves. What is never allowed is filling credentials or confirming an authorization on the user’s behalf.')
  }
