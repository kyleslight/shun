...
test('the slash palette exposes only useful implemented task commands with keyboard selection', async () => {
  ...
  assert.doesNotMatch(catalog, /id: "rename"[^\n]*args: true/)
  ...
  assert.match(app, /if \(!renameTarget\) return;[\s\S]*renameInput\.current\?\.focus\(\);[\s\S]*renameInput\.current\?\.select\(\);[\s\S]*\[renameTarget\?\.id\]/)
  assert.match(app, /ref=\{renameInput\}[\s\S]*autoFocus/)
