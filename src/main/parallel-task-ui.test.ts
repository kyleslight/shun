...
test('provider settings do not claim connectivity without a real probe', async () => {
  ...
  assert.match(app, /function localizedName\(value: \{ name: string; nameZh\?: string \} \| undefined, zh: boolean\)/)
  assert.match(app, /function configuredProviderName\(provider: Provider, catalog: ProviderCatalog \| null, zh: boolean\)/)
  assert.match(app, /candidate\.name === provider\.name \|\| candidate\.nameZh === provider\.name/)
  assert.match(app, /<b>\{localizedName\(provider, zh\)\}<\/b>/)
  assert.match(app, /provider\.variants\?\.map\(\(variant\) => localizedLabel\(variant, zh\)\)/)
  assert.match(app, /\{localizedLabel\(variant, zh\)\}/)
  assert.match(app, /let name = localizedName\(setupCatalogVariant, zh\) \|\| localizedName\(catalogProvider, zh\)/)
...
test('plugin hub exposes only implemented product capabilities', async () => {
  ...
  // One rail entry; Plugins and Skills are tabs inside the section, not rail entries.
  assert.doesNotMatch(sidebarNav, /zh \? "技能" : "Skills"/)
  assert.match(app, /plugin-hub-tabs[\s\S]*t\("Plugins", "插件"\)[\s\S]*t\("Skills", "Skill"\)/)
  // One page, two states of the same thing: what you have, then what you can add.
  assert.match(app, /installedPlugins = searchedPlugins\.filter\(\(plugin\) => plugin\.installed\)/)
  assert.match(app, /availablePlugins = searchedPlugins\.filter\(\(plugin\) => !plugin\.installed\)/)
  assert.doesNotMatch(app, /Plugins that ship with Shun|plugin-hub-filters/)
  // One grid for both sources keeps the two-per-line flow instead of each source
  // starting its own column.
  assert.match(app, /available = \[\s*\.\.\.availablePlugins\.map\(\(plugin\) => \(\{ kind: "plugin" as const, plugin \}\)\),\s*\.\.\.communityResults\.map\(\(entry\) => \(\{ kind: "community" as const, entry \}\)\),\s*\]/)
  assert.doesNotMatch(app.slice(app.indexOf('function PluginHub'), app.indexOf('function PluginLogo')), /plugin-publisher-row/)
  ...
  // A catalog with no last page: the section says how much exists and the reader
  // asks for the next window instead of the app downloading everything.
  assert.match(app, /const STORE_PAGE_SIZE = 20;/)
  assert.match(app, /storeResults\.length < storeTotal[\s\S]*loadMoreStore\(\)/)
  assert.match(app, /searchPluginMarketplace\(storeQuery, \{ limit: STORE_PAGE_SIZE, offset: storeResults\.length \}\)/)
  ...
  assert.match(css, /\.installed-section \.plugin-catalog-grid \{ padding-top: 13px; \}/)
  assert.match(css, /\.plugin-dialog-buttons \{[^}]*justify-self:end;/)
  assert.match(css, /\.plugin-store-updates,\.plugin-withdrawn-row \{[^}]*margin: 12px 0 0;/)
  assert.match(app, /<footer><span \/><div class="plugin-dialog-buttons">/)
  ...
  // Tabs are destinations on the toolbar's own hairline, not buttons in a box.
  assert.match(css, /\.plugin-hub-tabs button\.active \{[^}]*color: var\(--text-1\);[^}]*font-weight: 560;/)
  assert.match(css, /\.plugin-hub-tabs button\.active:after \{[^}]*background: var\(--text-1\);/)
  assert.doesNotMatch(css, /\.plugin-hub-tabs \{[^}]*border: 1px solid/)
  assert.match(css, /\.plugin-hub-toolbar \{[^}]*height: 48px;[^}]*border-bottom: 1px solid var\(--border-1\);/)
  assert.match(css, /\.plugin-hub-actions \{[^}]*margin-left: auto;/)
  assert.match(css, /\.installed-section \.plugin-catalog-grid \{ padding-top: 13px; \}/)
  assert.match(css, /\.plugin-hub-search \{[^}]*height: 38px;[^}]*border-radius: 19px;/)
...
test('streaming follows without fighting user scrolling and resumes near the latest content', async () => {
  ...
  assert.match(app, /direction,[\s\S]*nearEnd,[\s\S]*streaming: Boolean\(feedRunning\)/)
...
test('the sidebar resizes without breaking the conversation and plugin grid', async () => {
  ...
  // A plugin being developed is one row of the same list, labelled by its origin.
  assert.match(app, /t\("Development", "开发"\)/)
