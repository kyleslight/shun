        ...
        <section class="plugin-dialog" role="dialog" aria-modal="true" aria-label={selected.name} onPointerDown={(event) => event.stopPropagation()}>
          ...
          {!installation ? <div class="plugin-dialog-body"><p>{selected.description}</p>{selected.source === "installed" && <small>{t("This development package is registered with Shun but currently disabled.", "这个开发 Package 已注册到 Shun，但当前未启用。")}</small>}<button class="plugin-primary" onClick={() => install(selected)}>{selected.source === "installed" ? t("Enable", "启用") : t("Install", "安装")}</button></div> : <>
            <div class="plugin-dialog-body">
              ...
              {selected.id === "browser-use" && connectionState?.connected && <div class="plugin-connection-row plugin-enabled-row"><span><b>{t("Fall back to my Chrome for search", "用我的 Chrome 兜底检索")}</b><small>{t("Public sources cannot reach a company register, a signed-in repository, or an engine that refuses this machine; a few queries then run in your connected Chrome, strongest engine first, and their results are labelled as coming from your session. On while this plugin is connected — turn it off to keep every query in Shun's own public sources.", "公开来源到不了企业登记、需登录的资料库、或拒绝本机的搜索引擎；此时会用你已连接的 Chrome 发出少量查询（从最强引擎开始），结果标注来自你的会话。插件连接期间默认开启——关闭后所有查询只走 Shun 的公开来源。")}</small></span><label class="plugin-switch"><input aria-label={t("Fall back to my Chrome for search", "用我的 Chrome 兜底检索")} type="checkbox" checked={settings.browserSearchFallback === true} onChange={(event) => setSettings((current) => ({ ...current, browserSearchFallback: event.currentTarget.checked }))} /><i /><span>{settings.browserSearchFallback ? t("On", "已开启") : t("Off", "已关闭")}</span></label></div>}
