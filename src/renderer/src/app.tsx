        ...
        <section class="plugin-dialog" role="dialog" aria-modal="true" aria-label={selected.name} onPointerDown={(event) => event.stopPropagation()}>
          ...
          {!installation ? <div class="plugin-dialog-body"><p>{selected.description}</p>{selected.source === "installed" && <small>{t("This development package is registered with Shun but currently disabled.", "这个开发 Package 已注册到 Shun，但当前未启用。")}</small>}<button class="plugin-primary" onClick={() => install(selected)}>{selected.source === "installed" ? t("Enable", "启用") : t("Install", "安装")}</button></div> : <>
            <div class="plugin-dialog-body">
              ...
              {selected.id === "browser-use" && connectionState?.connected && <div class="plugin-connection-row plugin-enabled-row"><span><b>{t("Fall back to my Chrome for search", "用我的 Chrome 兜底检索")}</b><small>{t("When Shun's public sources come back thin, a few queries run in your connected Chrome and their results are labelled as coming from your session. Off by default: these queries use your own search account.", "公开来源返回很少时，用你已连接的 Chrome 发出少量查询，结果会标注来自你的会话。默认关闭：这些查询会使用你自己的搜索账号。")}</small></span><label class="plugin-switch"><input aria-label={t("Fall back to my Chrome for search", "用我的 Chrome 兜底检索")} type="checkbox" checked={settings.browserSearchFallback === true} onChange={(event) => setSettings((current) => ({ ...current, browserSearchFallback: event.currentTarget.checked }))} /><i /><span>{settings.browserSearchFallback ? t("On", "已开启") : t("Off", "已关闭")}</span></label></div>}
