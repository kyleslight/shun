...
export function App() {
  ...
  const task = tasks.find((x) => x.id === currentId) || tasks[0],
    ...
    zh = uiLanguage === "zh",
    // Remote is a mode of this surface, not a page beside it: the sidebar lists the
    // other machine's tasks, the feed draws its turns, the composer sends to it.
    remote = useRemoteSession({ language: resolveUiLanguage(settings.language), notify }),
    ...
    groups = taskGroups(visible, showRemote ? remoteRunningByTask : runningByTask),
    feedTurns = showRemote ? remoteTurnsAsLocal(remote.view, remote.records) : turns,
  ...
  function isRunning(item: Task) {
    if (showRemote) return Boolean(remoteRunningByTask[item.id]);
              ...
              <div class={`workspace-tasks ${group.workspace && collapsedWorkspaces.includes(group.workspace) ? "collapsed" : ""}`}>
                {groupTasks.map((item) => (
                  <div class="task-row" key={item.id}>
                    <button
                      class={`task ${showRemote ? (item.id === remote.open?.taskId ? "active" : "") : (!showPlugins && !showSchedules && item.id === currentId ? "active" : "")}`}
                      onClick={() => showRemote ? remote.openTask(remote.activeId, item.id) : selectTask(item)}
                    >
        ...
        {showSchedules ? (
          ...
          <>
            <header>
              ...
              {!!feedTurns.length && (
                <div class="task-heading" title={showRemote ? (remote.activeTask?.title || "") : (task?.title || (zh ? "新建任务" : "New task"))}>
                <strong>{showRemote
            ...
            <div
              class={`feed ${runLayoutTask.current === feedKey ? "run-anchored" : ""} ${!feedTurns.length ? "empty-state" : ""}`}
              ref={feed}
              onScroll={(e) => {
            ...
            >
              ...
              {!!feedTurns.length && (
                <TaskHistory
                  key={feedKey}
                  turns={feedTurns}
                  attachments={showRemote ? [] : (task?.attachments || [])}
                  workspace={feedWorkspace}
                  running={feedRunning}
                  language={uiLanguage}
                  retry={showRemote ? undefined : retry}
                  revise={showRemote ? undefined : revisePrompt}
                  copyText={copyText}
                  ...
                  placeholder={
                    showRemote
                      ? (remote.open
                        ...
                        : (zh ? `在 ${remote.active?.name || "那台 Shun"} 上开始新任务…` : `Start a new task on ${remote.active?.name || "that Shun"}…`))
                      : task?.workspace
...
function RemoteDiffLines({ hunks, zh }: { hunks: RemoteDiffHunk[]; zh: boolean }) {
  ...
  // The same line vocabulary the app's own diff view uses, so a remote diff and a
  // local one read identically.
  return <div class="inline-diff">
    {hunks.map((hunk, index) => <Fragment key={`${hunk.header}-${index}`}>
      <div class="hunk"><span>@</span><code>{hunk.header}</code></div>
      {hunk.lines.map((line, position) => <div key={position} class={line.type === "add" ? "add" : line.type === "remove" ? "del" : "context"}>
...
/**
 * One tool call on the other Shun, drawn with this app's own activity rows.
 *
 * The streamed row carries a bounded preview on purpose; opening it asks the
 * peer for the whole record — arguments, output, and diff — which is where a
 * long build log or a change actually becomes readable.
 */
function RemoteToolRow({ tool, record, zh, expanded, toggle }: { tool: RemoteTool; record?: RemoteToolRecord | null; zh: boolean; expanded: boolean; toggle: () => void }) {
  const kind = toolGroupKind({ id: tool.id, name: tool.name, input: "", state: tool.state === "running" ? "running" : tool.state === "error" ? "error" : "done" });
  const Icon = kind === "research" || kind === "inspection" ? Search : kind === "change" ? FilePenLine : SquareTerminal;
  const title = remoteToolTitle(tool, zh);
 ...
 */
/**
 * The surfaces that exist only because the task is on another machine: the
 * peer's changes, its workspace, its processes, its terminal, and the pairing
 * that attaches one.
 *
  ...
  return <>
    ...
    {!!browsing && <div class="remote-browse">
      ...
      <div class="remote-browse-list">
        {browsing.parent && <button onClick={() => void session.browseWorkspace(browsing.parent)}><ArrowUp />..</button>}
        {browsing.entries.map((entry) => <button key={entry.path} onClick={() => void session.browseWorkspace(entry.path)}><Folder />{entry.name}</button>)}
...
function TaskHistory({
  ...
  language: UiLanguage;
  hitTurnId?: string;
  /** Absent when the conversation is not this machine's to edit or retry. */
            ...
            return (
              ...
              {editing?.id === turn.id ? (
                <form
                  ...
                  onSubmit={(event) => {
                    ...
                    setEditing(null);
                    // Editing exists only where a caller can act on it; a remote
                    // conversation is not this machine's transcript to revise.
                    if (revise) void revise(turn.id, next);
                  }}
                >
                  <textarea
                    autoFocus
              ...
              {editing?.id !== turn.id && turn.content && turn.id !== running && (turn.role === "user" || turn.completedAt) && (
                <div class="turn-actions">
                  ...
                  {turn.role === "assistant" && turn.error && retry && (
...
function Tool({
  ...
  attachmentNames: ReadonlyMap<string, string>;
  recovered?: boolean;
  /** Asked for the whole record the first time the row is opened, where a caller has one. */
  ...
  return (
    <div class={`tool-row ${recovered ? "state-done" : `state-${tool.state}`}`} data-tool-id={tool.id}>
      <button class="tool-row-head" onClick={toggleOpen}>
        <Icon />
        <span>
...
function PluginHub({
  ...
  const [plugins, setPlugins] = useState<PluginState[]>([]),
    ...
    [gmailOwnClient, setGmailOwnClient] = useState(false),
  ...
  const findInstallation = (pluginId: string) => (value.plugins || []).find((item) => item.id === pluginId),
    ...
    connect = async (plugin: PluginState) => {
      ...
      if (plugin.id === "gmail" && !gmailOAuthClient.trim() && !plugin.connector.authorizeLabel) {
      ...
      try {
        const credential = plugin.id === "figma" ? figmaToken.trim() : plugin.id === "gmail" ? gmailOAuthClient.trim() || undefined : plugin.id === "render" ? renderApiKey.trim() : plugin.id === "cloudflare" ? cloudflareApiToken.trim() : undefined;
        ...
        if (result.connected && plugin.id === "gmail") { setGmailOAuthClient(""); setGmailOwnClient(false); }
  ...
  return <>
    ...
    {selected && (() => {
      const installation = findInstallation(selected.id), enabled = Boolean(installation) && installation?.enabled !== false,
        ...
        connectionState = connection[selected.id], credentialPlugin = selected.connector.auth === "pat" || selected.connector.auth === "oauth" || selected.connector.auth === "api-key", hostAuthorization = selected.connector.authorizeLabel, localPlugin = selected.connector.auth === "local",
        ...
        connectionDescription = selected.id === "github"
          ...
          : selected.id === "gmail"
            ? hostAuthorization
              ? t("Uses Google desktop OAuth with the Gmail modify scope and Shun's own Google client. OAuth tokens stay encrypted by the operating system; permanent deletion is not exposed.", "使用 Google 桌面 OAuth 和 Gmail modify Scope，并用 Shun 自己的 Google Client 授权；OAuth Token 由操作系统加密保存，且不提供永久删除能力。")
        ...
        <section class="plugin-dialog" role="dialog" aria-modal="true" aria-label={selected.name} onPointerDown={(event) => event.stopPropagation()}>
          ...
          {!installation ? <div class="plugin-dialog-body"><p>{selected.description}</p>{selected.source === "installed" && <small>{t("This development package is registered with Shun but currently disabled.", "这个开发 Package 已注册到 Shun，但当前未启用。")}</small>}<button class="plugin-primary" onClick={() => install(selected)}>{selected.source === "installed" ? t("Enable", "启用") : t("Install", "安装")}</button></div> : <>
            <div class="plugin-dialog-body">
              ...
              {selected.id === "browser-use" && connectionState?.connected && fastBrowserEnabled && <div class="plugin-connection-row plugin-enabled-row"><span><b>{t("Decision service", "决策服务")}</b><small>{t("Which service answers the fast decisions. Auto uses a credential you already configured, preferring OpenRouter, then TypeSafe, then Vercel AI Gateway.", "由哪个服务回答快速决策。自动会优先使用你已配置的凭证，顺序为 OpenRouter、TypeSafe、Vercel AI Gateway。")}</small></span><span class="plugin-select-control"><select aria-label={t("Decision service", "决策服务")} value={fastBrowserChoice} onChange={(event) => update((current) => ({ ...current, computerUseAcceleration: { ...current.computerUseAcceleration, provider: event.currentTarget.value === "auto" ? undefined : event.currentTarget.value } }))}><option value="auto">{t("Auto", "自动")}</option>{Object.values(decisionRoutes).map((route) => <option value={route.id}>{route.label}</option>)}{(value.providers || []).filter((item) => decisionRouteForEndpoint(item.endpoint)).map((item) => <option value={item.id}>{`${item.name}${t(" · this machine", " · 本机配置")}`}</option>)}</select><ChevronDown aria-hidden="true" /></span></div>}
            ...
            {/* One footer, not a stack of them: the notes read on the left and every action for this
                plugin sits on the right, so a lone button never floats in a band of its own. */}
            {(enabled && !!selectedViews.length) || previousVersion || selected.reloadable ? <footer class="plugin-dialog-footer">
              <div class="plugin-dialog-footer-notes">
                {previousVersion && <small>{t(`Version ${previousVersion.version} is still on disk`, `v${previousVersion.version} 仍在磁盘上`)}</small>}
                {selected.reloadable && <small title={selected.developmentSource}>{t("Linked development source", "已关联开发源")} · {selected.developmentSource}</small>}
              </div>
              <div class="plugin-dialog-footer-actions">
                {previousVersion && <button disabled={connecting === selected.id} onClick={() => void restorePrevious()}><RotateCcw />{t(`Restore v${previousVersion.version}`, `回退到 v${previousVersion.version}`)}</button>}
                {selected.reloadable && <button disabled={connecting === selected.id} onClick={() => void reloadPluginPackage(selected)}>{connecting === selected.id ? <LoaderCircle class="loading-spinner" /> : <RotateCcw />}{t("Apply changes", "应用改动")}</button>}
                {enabled && selectedViews.map((view) => <button class="plugin-primary" onClick={() => openView(view)}><PanelRightOpen />{view.title}</button>)}
              </div>
            </footer> : null}
...
/** Recents are the long tail of standalone work and page by 50; a project stays a short, scannable list. */
const recentsTaskPageSize = 50,
  workspaceTaskPageSize = 5;
function sidebarTaskPageSize(workspace: string) { return workspace ? workspaceTaskPageSize : recentsTaskPageSize; }
