...
- `license`, `homepage`, `repository`, `keywords`: store metadata. Fill these in before publishing — the store page shows the license, and keywords are how someone finds the plugin. `license` is an SPDX identifier, the two URLs must be credential-free HTTPS, and `keywords` holds at most 8 unique labels of 1-32 characters.
