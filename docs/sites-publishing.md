...
**slug 规则**：项目目录名 kebab 化，`[a-z0-9-]`，3–40 字符；保留字（`www`/`api`/`mail`/`cdn`/`admin`/`_dmarc`/`*`/`@` 等）；
重名递增 `-2`；发布前检查 zone 内是否已有同名 DNS 记录，有则拒绝并给替代名（绝不静默抢占用户既有子域）。
预览型发布可用随机 slug（`quiet-lake-3f2a`），与管理型 slug 分两个命名空间。
